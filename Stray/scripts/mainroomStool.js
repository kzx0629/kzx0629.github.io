window.onload = function () {
    ...
    //获取localStorage中的值
    let stoolX = localStorage.getItem('mainroomStoolX') || '71px';
    let stoolY = localStorage.getItem('mainroomStoolY') || '352px';
    ...
    drag.onmousedown = function (e) {
        ...
        document.onmouseup = function (e) {
            ...
            this.onmouseup = null;
            localStorage.setItem('mainroomStoolX', drag.style.left)
            localStorage.setItem('mainroomStoolY', drag.style.top)
            localStorage.setItem('flashlightStoolFlag', 'false')
            zones.forEach(zone => {
                ...
                if (draggableRect.left < rect.right && draggableRect.right > rect.left && draggableRect.top < rect.bottom && draggableRect.bottom > rect.top) {
                    if (zone.id == 'flashlight') {localStorage.setItem('flashlightStoolFlag', 'true')}
                    switch (zone.id){
                        case 'corridor': {
                            if (String(localStorage.getItem('getKeyFlag')) != 'true') return;
